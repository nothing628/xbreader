  ...
  implements ClassComponent<ReflowableSpineAttrs>
{
  ...
  oninit({ attrs }: CVnode<ReflowableSpineAttrs>) {
    Promise.all(
      attrs.spine.map(
        (link) =>
          new Promise<PData>(
            (resolve: (d: PData) => void, reject: (e: Error) => void) => {
              const handler = (e: MessageEvent) => {
                if (e.data.src === link.Href) {
                  ...
                  if (e.data.url) {
                      ...
                      fetch(e.data.url)
                        ...
                        .then((plaintext) => {
                          const doc = new DOMParser().parseFromString(
                            plaintext,
                            link.TypeLink as DOMParserSupportedType
                          );
                          ...
                          Array.from(
                            ...
                            for (let i = 0, j = 0; i < nodes.length; i = j) {
                              ...
                              if (
                                sections.length > 0 &&
                                sections[sections.length - 1]?.innerHTML.trim()
                                  .length === 0
                              ) {
                                section = sections[
                                  sections.length - 1
                              ...
                              if (
                                SEPARATE_ELEMENTS.includes(
                                  nodes[j].tagName?.toUpperCase()
                                )
                              ) {
                    ...
                    else resolve({ blob: e.data.url, link });
  ...
  view({ attrs }: CVnode<ReflowableSpineAttrs>) {
    ...
    return m(
      "div#br-spine.reflowable" + (slider.single ? ".single" : ".double"),
      {
        style: slider.properties,
        key: `reflowable-spine-${slider.perPage}`,
        onupdate: (vnode) => {
          slider.rlength = Math.max(
            1,
            Math.ceil(
              vnode.dom.scrollWidth / (vnode.dom as HTMLElement).offsetWidth
            )
          );
        },
