...
export default class ReflowablePage
  implements ClassComponent<ReflowablePageAttrs>
{
  ...
  view(vnode: Vnode<ReflowablePageAttrs, this>) {
    ...
    if (slider) {
      ...
      if (slider.single || slider.ttb) spread = false;
    } else
      docWidth =
      ...
      innerItemAttrs.onload = (e: Event) => {
        ...
        if (tx) {
          ...
          tx.addEventListener("pointermove", (e) => {
            (e as unknown as MithrilEvent).special = true;
