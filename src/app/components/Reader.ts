...
import {
  worker as workerPool,
  f as workerFunc,
  chooserFunction,
} from "xbreader/helpers/lazyLoader";
...
export default class Reader implements ClassComponent<ReaderAttrs> {
  ...
  constructor(vnode: Vnode<ReaderAttrs>) {
    ...
    if (sML.Mobile) this.mobile = true;
  ...
  // IGNORE: New Reader doesn't use this function
  ...
  switchDirection(direction?: XBReadingDirection | string) {
    ...
    if (direction === undefined) {
      ...
      if (pdir === XBReadingDirection.TTB) {
        // Should not be encountered
        console.error(
          "Vertical publications cannot be switched to a horizontal layout!",
        );
        return false;
      }
  ...
  oninit() {
    // If the pool was previousy destroyed
    // Create new one
    if (workerPool?.destroyed)
  ...
  onremove() {
    // Slider and binder
    if (this.binder) this.binder.destroy();
  ...
  oncreate(vnode: VnodeDOM<ReaderAttrs, this>) {
    ...
    if (!manifestPointer)
      ...
      else manifestPointer = vnode.attrs.cid + ".json";

    if (!manifestPointer) {
      console.warn("No item specified");
      m.route.set(
        "/error/:code/:message",
        { code: 9400, message: t`No item specified` },
        { replace: true },
      );
      return;
    }
    this.updateStatus(t`Fetching info...`);
    // this.setTitle(t`Loading...`); // IGNORE:
    this.publication
      ...
      .then(() => {
        ...
        m.redraw();
        window.setTimeout(() => {
          if (this.ui && !this.ui.mousing) this.ui.toggle(false);
          m.redraw();
        }, 1500);
        // IGNORE:
      ...
      .catch((error: xbError | Error) => {
        if (typeof (error as xbError).export === "function") {
          ...
          const encodedMessage = encodeURIComponent(
            window.btoa(error.message ? error.message : error.toString()),
          );
          m.route.set(
            "/error/:code/:message",
            {
              code: (error as xbError).code ? (error as xbError).code : 9500,
  ...
  view(vnode: Vnode<ReaderAttrs, this>) {
    ...
    // IF publication not exists or publication exists but not ready
    // Show Loading screen
    ...
    // If slider is reflowable and book direction not TTB, add height property to bookStyle
    if (sldr.reflowable && !sldr.ttb) {
      Object.assign(bookStyle, {
        height: "100%",
      });
    }

    document.documentElement.style.overflow = sldr
      ? sldr.ttb || !sldr.spread
        ? "auto"
        : "hidden"
      : "hidden";
    ...
    if (bnd) {
      ...
      if (
        sML.OS.Windows &&
        !(sML.UA.Firefox && window.devicePixelRatio > 1) &&
        (cursr.endsWith("-resize") || cursr === "context-menu")
      )
        bookClass += ` cursor__${bnd.cursor}`;
      else bookStyle.cursor = cursr;
    }
    ...
    // If publication is ready and slider model ready
