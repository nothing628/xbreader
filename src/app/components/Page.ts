...
export default class Page implements ClassComponent<PageAttrs> {
  ...
  get float() {
    ...
    if (this.data.Properties.Page === "left") return "right";
    if (this.data.Properties.Page === "right") return "left";
  ...
  oninit(vnode: Vnode<PageAttrs, this>) {
    ...
    if (vnode.attrs.blank) this.blank = true;
    else
      this.loader = new SmartLoader(
        this.data,
        vnode.attrs.slider.publication,
        vnode.attrs.index,
        vnode.attrs.renderConfig,
        vnode.attrs.chooseCallback
      );
  }
  ...
  view(vnode: Vnode<PageAttrs, this>) {
    ...
    if (slider) {
      ...
      if (slider.single || slider.ttb || this.landscape) spread = false;
    ...
    if (
      this.itemWidth * 2 > docWidth &&
      docHeight <= docWidth &&
      (!this.landscape || spread)
    ) {
    ...
    if (slider && (slider.ttb || slider.single)) {
      ...
      if (slider.toon || this.data.Height / this.data.Width > 2) {
        // TTB publication or very tall image
        // ... && this.data.Width < docWidth
        if (
          slider.publication.isTtb ||
          ((this.data.Height > MAX_FIT_HEIGHT ||
            this.data.Width > MAX_FIT_WIDTH) &&
            !(this.landscape && !slider.toon))
        ) {
          // Too large to fit, compromise with maxFit
          const preferredWidth = Math.min(
            docWidth,
            slider.fit ? MIN_TTB_WIDTH : MAX_TTB_WIDTH
          );
          if (preferredWidth < this.data.Width) {
            this.itemHeight =
              (preferredWidth / this.data.Width) * this.data.Height;
        ...
        if (!slider.fit && vnode.attrs.isImage && docHeight <= docWidth) {
          // Landscape window. Fit to original size
          const mFitWidth =
            (MAX_FIT_WIDTH / this.data.Height) * this.data.Width;
          if (
            this.data.Height > MAX_FIT_WIDTH &&
            !this.landscape &&
            mFitWidth < docWidth
          ) {
            // Too large to fit, compromise with maxFit
        ...
        if (slider.ttb) this.marginTop = vnode.attrs.index > 0 ? 10 : 0; // this.marginTop = this.loader.reloader ? this.data.Height : (vnode.attrs.index > 0 ? 10 : 0);
      }
      itemAttrs.style = `height: ${this.parseDimension(
        this.itemHeight
      )}; width: ${this.parseDimension(this.itemWidth)}; margin: ${
        this.marginTop
      }px auto 0 auto;`;
    } else {
      // Horizontal (LTR & RTL)
      if (this.data.findFlag("addBlank"))
        if (slider.rtl)
          // If item is single with full width, position properly
    ...
    if (vnode.attrs.blank)
      ...
      if (vnode.attrs.isImage) {
        ...
        if (willDraw) innerItemAttrs.height = innerItemAttrs.width = 0;
        else innerItemAttrs.decoding = "async";
        innerItemIs = m(
          (willDraw ? "canvas" : "img") + ".page-img.noget.noselect",
          innerItemAttrs
        );
      } else {
        ...
        innerItemAttrs.onload = (e: Event) => {
          if (!(e.target as HTMLIFrameElement).src)
            // Not loaded with document, just empty
            return;
          ...
          tx.addEventListener(
            "touchstart",
            vnode.attrs.binder.touchstartHandler
          );
