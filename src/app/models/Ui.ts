...
export default class Ui {
  ...
  toggleMenu(newState?: boolean) {
    if (newState === undefined) this.menuShown = !this.menuShown;
    else if (this.menuShown !== newState) this.menuShown = newState;
  }

  toggleSettings(newState?: boolean) {
    if (newState === undefined) this.settingsShown = !this.settingsShown;
    else if (this.settingsShown !== newState) this.settingsShown = newState;
    else return;

    if (this.settingsShown) this.toggleMenu(false);
  }
  ...
  toggle(newState?: boolean) {
    if (newState === undefined) this.isHidden = !this.isHidden;
    ...
    if (this.toggleCallback) this.toggleCallback(!this.isHidden);
    return true;
  }
}
