...
import { JsonStringConverter } from "r2-utils-js/dist/es5/src/_utils/ta-json-string-converter";
...
export default class Publication {
  ...
  smartLoad(item: any) {
    ...
    if (
      ...
      // Is a successful Promise
      return item
        .then((data: any) => this.loadFromData(this.parseManifest(data)))
        .catch((error: Error) => {
          throw error;
        });
    }
    if (typeof item === "string" /* || item instanceof String*/)
      // Is a string (URL we hope)
      return this.loadFromPath(item);
    else
      try {
        // We can't tell if the item is a rejected promise, so we try and catch it, and if that fails, assume it's an object
        return item.catch(
          (err: Error) =>
  ...
  parseManifest(rawManifest: any) {
    if (!rawManifest) throw new xbError(9400, t`Manifest data empty`);
    if (rawManifest instanceof Publication) return rawManifest;
    let md;
    if (typeof rawManifest === "string") md = JSON.parse(rawManifest);
    else md = rawManifest;
  ...
  loadFromPath(manifestPath: string) {
    return m
      .request<Publication>({
        ...
        extract: (xhr, _) => {
          const success =
            ...
            xhr.status === 304 ||
            /^file:\/\//i.test(manifestPath);
          if (!success) throw new xbError(xhr.status);
      ...
      .catch((error) => {
        if (error instanceof xbError) throw error;
  ...
  static fixDeprecated(container: any, oldKey: string, newKey: string) {
    if (oldKey in container === true && newKey in container === false) {
      console.warn(
        `Deprecated WebPub naming scheme: Change '${oldKey}' to '${newKey}'`,
      );
  ...
  private isValidManifest(manifest: any) {
    ...
    if (!this.keysInObj(requiredMetadataKeys, manifest.metadata)) return false;
  ...
  get isSmallToon() {
    if (this.smallToon !== -1)
      // Memoized, since used in render loop
    ...
    if (smallestWidth <= MIN_TTB_WIDTH + 50) this.smallToon = 1;
    else this.smallToon = 0;
    return this.smallToon;
  }
}
