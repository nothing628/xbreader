...
export default class Config {
  ...
  public saveSettings(): boolean {
    try {
      ...
      this.settings.forEach(
        (setting) => (exportedSettings[setting.name] = setting.value)
      );
      let storeLocal = true;
  ...
  get animate(): boolean {
    ...
    if (this.internalAnimateSettingIndex === -2)
      this.internalAnimateSettingIndex = this.settings.findIndex(
        (s) => s.name === "animations"
      );
    if (this.internalAnimateSettingIndex === -1) return true;
    return this.settings[this.internalAnimateSettingIndex].value === "off"
      ? false
      : true;
  }
  ...
  get background(): string {
    ...
    if (this.internalBackgroundSettingIndex === -2)
      this.internalBackgroundSettingIndex = this.settings.findIndex(
        (s) => s.name === "background"
      );
