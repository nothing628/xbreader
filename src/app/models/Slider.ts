...
export default class Slider {
  ...
  resizeHandler(slide = true, fast = true) {
    ...
    this.updateProperties(true);
    if (slide && !sML.Mobile) this.slideToCurrent(!fast, fast);
  ...
  get reflowableMargin() {
    return (this.fit && this.ttb) || (this.single && this.fit)
      ? this.width > DEFAULT_MARGIN * 2 + MAX_MARGIN_WIDTH
        ? (this.width - MAX_MARGIN_WIDTH) / 2 + DEFAULT_MARGIN
        : DEFAULT_MARGIN
      : DEFAULT_MARGIN;
  }
  ...
  updateProperties(animate: boolean, fast = true) {
    ...
    if (this.perPage > 1 && this.shift) margin = `${this.width / 2}px`;
    if (this.ttb) {
      this.properties = {};
    } else {
      this.properties = {
    ...
    if (this.reflowable && !this.ttb) {
      ...
      this.properties.columns = `${
        (this.width - this.perPage * margin * 2) / this.perPage
      }px ${this.perPage}`;
  ...
  get portrait(): boolean {
    if (this.orientationInternal === -1) {
      this.orientationInternal =
        this.innerHeightCached > window.innerWidth ? 1 : 0;
    }
  ...
  get length() {
    if (this.single || this.reflowable) return this.slength;
  ...
  toggleFit() {
    this.fit = !this.fit;
    if (this.publication.isScrollable && this.ttb)
      this.slideToCurrent(false, true);
  ...
  onChange() {
    if (this.binder) this.guideHidden = true;
    this.zoomer.scale = 1;
    m.redraw();
    ...
    this.PageChangeTimer = window.setTimeout(
      () =>
        this.config.state.onPageChange(
          this.outsidePageNumber(),
          this.direction,
          !this.single
        ),
  ...
  next(howManySlides = 1) {
    ...
    if (
      ...
      this.currentSlide + 1 === this.length
    ) {
   ...
   */
  goTo(index: number) {
    if (this.slength <= this.perPage && !this.reflowable) return;
  ...
  private get offset() {
    return (
      (this.rtl ? 1 : -1) * this.currentSlide * (this.width / this.perPage)
    );
  }
  ...
  get percentage(): number {
    ...
    if (this.ttb) {
      ...
      const tot =
        this.binder.coordinator.HTML.scrollTop +
        this.binder.coordinator.Body.scrollTop;
  ...
  slideToCurrent(enableTransition?: boolean, fast = true, changed = true) {
    ...
    if (this.ttb) {
      if (this.toonflowable) {
        // Is a TTB publication
        ...
        // Is not a TTB publication (but being read in TTB mode)
        const br_slider = this.selector;
        if (!br_slider) return;
        const children = br_slider.children;
        requestAnimationFrame(() => {
          // TODO nicer animation. Right now you still see a flash of the top
          requestAnimationFrame(() => {
    ...
    if (this.single && !this.ttb && changed)
      // Scroll back to top for next/prev page
      window.setTimeout(() => {
