...
export default class Series {
  ...
  set current(newChapter: XBChapter) {
    ...
    this.chapters?.forEach((chapter) => {
      if (chapter.selected) chapter.selected = false;
      if (chapter.uuid === newChapter.uuid) chapter.selected = true;
    });
    this.volumes?.forEach((volume) => {
      volume.chapters?.forEach((chapter) => {
        if (chapter.selected) chapter.selected = false;
  ...
  get selector(): Children {
    if (this.isSolo)
      if (this.publication.pmetadata.Title)
        return m(
          "span.br-toolbar__ellipsis#br-chapter",
          {
            title: this.publication.pmetadata.SubTitle
              ? this.publication.pmetadata.SubTitle
              : t`Chapter`,
          },
    ...
    const select = m(
      ...
      this.volumes.map((volume) => {
        ...
        volume.chapters.forEach((chapter) => {
          ...
          if (isSel && volume.title && volume.title.trim())
            selectedVolumeName = m(
              "span#br-chapter__volume",
              {
    ...
    return m("span#br-chapter__group", [
      selectedVolumeName
        ? [selectedVolumeName, m("span.spacer", spacer)]
        : null,
      select,
    ]);
  }
