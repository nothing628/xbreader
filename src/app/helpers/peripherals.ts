...
export default class Peripherals {
  ...
  onmessage(e: MessageEvent) {
    ...
    if (e.data.indexOf("xbr:") !== 0) return;
    e.stopPropagation();
      ...
      case 2: {
        ...
        if (this.pointerDown)
          this.pinch.startOffset = {
            X:
              this.slider.zoomer.translate.X && this.slider.zoomer.scale > 1
                ? this.slider.zoomer.translate.X
                : e.touches[0].pageX,
            Y:
              this.slider.zoomer.translate.Y && this.slider.zoomer.scale > 1
  ...
  mtimerUpdater(e: MithrilEvent) {
    // TODO mithril event type
    e.redraw = false;
    window.setTimeout(() => {
  ...
  touchmoveHandler(e: TouchEvent) {
    ...
    if (this.pointerDown && this.drag.letItGo) {
      ...
      const currentOffset =
        currentSlide * (this.slider.width / this.slider.perPage);
      const dragOffset = this.drag.endX - this.drag.startX;
      const offset = this.slider.rtl
        ? currentOffset + dragOffset
        : currentOffset - dragOffset;

      (this.slider.properties as any).transform = `translate3d(${
        (this.slider.rtl ? 1 : -1) * offset
      }px, 0, 0)`;
    }
  }
  ...
  private cursorHandler() {
    ...
    const atFirstSlide = this.slider.toonflowable
      ? this.slider.percentage === 0
      : Math.max(this.slider.currentSlide, 0) === 0;
    const atLastSlide = this.slider.toonflowable
      ? this.slider.percentage > 99.9
      : Math.min(this.slider.currentSlide, sliderLength) === sliderLength &&
      ...
      if (!e.special) {
        ...
        if (this.mousePosOut) {
          ...
          this.mtimer = window.setTimeout(() => {
            if (this.mousePosOut || this.ui.mousing) return;
  ...
  private changeCursor(newCursor: string) {
    if (newCursor === this.currentCursor) return;
  ...
  private updateKeyCodes(
    EventTypes: string[] | string,
    KeyCodesToUpdate: Record<string, any>
  ) {
    if (typeof (EventTypes as string[]).join !== "function")
      EventTypes = [EventTypes as string];
    if (typeof KeyCodesToUpdate === "function")
      KeyCodesToUpdate = KeyCodesToUpdate();
    (EventTypes as string[]).forEach((EventType) => {
      this.KeyCodes[EventType] = sML.edit(
        this.KeyCodes[EventType],
        KeyCodesToUpdate
      );
    });
  }
  ...
  private updateAfterDrag() {
    const movement =
      (this.slider.rtl ? -1 : 1) * (this.drag.endX - this.drag.startX);
    ...
    if (
      movement > 0 &&
      movementDistance > this.slider.threshold &&
      this.slider.slength > this.slider.perPage
    ) {
  ...
  onkeyup(Eve: BibiKeyboardEvent) {
    ...
    if (!this.onEvent(Eve)) return false;
    if (
      this.ActiveKeys[Eve.KeyName] &&
      Date.now() - this.ActiveKeys[Eve.KeyName] < 300
    ) {
  ...
  private tryMoving(Eve: BibiKeyboardEvent) {
    if (!Eve.KeyName) return false;
    const MovingParameter =
      this.MovingParameters[
        !Eve.shiftKey ? Eve.KeyName : Eve.KeyName.toUpperCase()
      ];
    if (!MovingParameter) {
  ...
  attemptScrollTo(offset: number) {
    if (
      this.coordinator &&
      this.coordinator.HTML &&
      this.coordinator.HTML.scrollTo
    )
      this.coordinator.HTML.scrollTo({
    ...
    else if (window.scrollTo) window.scrollTo(0, offset);
    else console.error("scrollTo not supported!");
  }
  ...
  moveBy(MovingParameter: any) {
    ...
    if (typeof MovingParameter === "number") {
      ...
      switch (MovingParameter) {
        ...
        case "zoom-out": {
          if (this.slider.zoomer.scale > 1) this.slider.zoomer.scale -= 0.5;
          break;
        }
        case "zoom-reset":
    ...
    if (MovingParameter === "menu") this.ui.toggle();
    else this.ui.toggle(false);
  ...
  private processVScroll() {
    if (this.ui.settingsShown) return true;
    if (!this.slider.ttb) return false;
    ...
    for (let index = 0; index < pages.length; index++) {
      ...
      if (
        ...
        if (index !== this.slider.currentSlide || this.slider.toon) {
          ...
          if (!this.ui.mousing) this.ui.toggle(false); // Hide UI when changing pages
  ...
  onwheel(Eve: BibiWheelEvent) {
    ...
    } else if (!this.slider.ttb && this.slider.spread) {
      // Vertical scrolling for horizontal view when not in single page mode
      CW.Distance = Eve.deltaY < 0 ? -1 : 1;
      CW.Delta = Math.abs(Eve.deltaY);
    } else {
      // Single page scroll instantly hides interface
    ...
    if (!PWs[PWl - 1]) {
      ...
      if (
        PWl >= 3 &&
        PWs[PWl - 2].Distance !== CW.Distance &&
        PWs[PWl - 3].Distance !== CW.Distance
      )
        CW.Wheeled = WheelState.Reverse;
    ...
    if (CW.Wheeled !== WheelState.None) {
      ...
      this.Timer_cooldown = window.setTimeout(
        () => (this.onwheeled_hot = false),
        SCROLL_COOLDOWN
      );
