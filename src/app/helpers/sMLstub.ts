...
class sML {
  ...
  constructor() {
    // userAgentData is experimental feature
    const NUAD = (navigator as any).userAgentData,
    ...
    this.Mobile =
      this.OS.iOS || this.OS.Android || this.OS.WindowsPhone ? true : false;

    this.UA = ((UA: UAFlags) => {
      let _OK = false;
      if (NUAD && Array.isArray(NUAD.brands)) {
        const BnV = NUAD.brands.reduce((BnV: any, _: any) => {
  ...
  edit(Obj: object | HTMLElement, ...ProSets: any) {
    ...
    if ((Obj as HTMLElement).tagName) {
      for (let i = 0; i < l; i++) {
        const ProSet = ProSets[i];
        for (const Pro in ProSet) {
          if (Pro === "$1" || Pro === "style") continue;
          if (/^data-/.test(Pro))
            (Obj as HTMLElement).setAttribute(Pro, ProSet[Pro]);
          else (Obj as any)[Pro] = ProSet[Pro];
        }
        if (ProSet.on)
          for (const EN in ProSet.on)
            (Obj as HTMLElement).addEventListener(EN, ProSet.on[EN]);
