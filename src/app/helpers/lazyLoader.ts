            ...
            if (xhr.status && xhr.status < 400) {
              if (e.data.bitmap && "createImageBitmap" in self) {
                ...
                try {
                  ...
                  ctx.postMessage({
                    src,
                    error: `Failed to load item ${src}: ${error}`,
                  });
                }
              } else {
              ...
              ctx.postMessage({
                src,
                error: `Failed to load item ${src}, status ${xhr.status}`,
              });
            }
            deqeue(src);
          };
...
export default class LazyLoader {
  ...
  constructor(
    ...
    if (renderConfig.onDraw) {
      ...
      itemData.Height &&
      itemData.Width &&
      (!renderConfig.bitmap || !this.drawer)
    ) {
      this.canvas = document.createElement("canvas");
    }

    if (!itemData.findFlag("isImage")) this.drawer = null; // No drawer for non-images (e.g. iframe)
  }
  ...
  provoke(imageItem: CVnodeDOM<LoadableElement>, currentIndex: number) {
    ...
    if (diff <= LOW_THRESHOLD) this.prepare(); // Start loading the image
    // If index of page near current page, load after predicted end of transition
      ...
      if (this.drawer && this.canvas) {
        ...
        else (this.element as HTMLIFrameElement).src = BLANK_PAGE;
      }
      this.loaded = false;
      this.reloader = true;
      m.redraw();
  ...
  destroy(): void {
    ...
    if (!this.loaded && workerSupported)
      this.worker.postMessage(
        { src: this.href, mode: "CANCEL" },
        [],
        this.specificWorker
      );
    if (this.blob && this.blob.indexOf("blob:") === 0)
  ...
  private toBlob(_type?: string, _quality?: number) {
    ...
    if (this.blob?.indexOf("data:") === 0) {
      ...
      this.canvas.toBlob((blob) => {
        ...
        if (!this.loaded) (this.element as HTMLImageElement).src = this.blob;
      });
  }
  ...
  private draw(element: HTMLElement | string, mel = false) {
    ...
    this.drawT = requestAnimationFrame(() => {
      ...
      if (
        ...
        this.blob = svgStatusGenerator(
          element,
          infoString,
          this.best.Width || this.element.width,
          this.best.Height || this.element.height
        );
        this.drawAsSoon();
        return;
      }
      ...
      if (!this.noContext) {
        try {
          ...
          if (!dctx) {
            ...
            ctx = tempCanvas.getContext(
              "2d",
              !sML.UA.Chrome || sML.UA.Chrome[0] >= 83
                ? { desynchronized: true }
                : null
  ...
  private drawAsSoon(first = false) {
    if (this.element && !first) {
      if (this.loaded) {
        if (!workerSupported && !this.drawer) {
          // C
          if (this.element instanceof HTMLImageElement)
            (this.element as HTMLImageElement).src = (
              this.preloader as HTMLImageElement
            ).src;
          else {
            // Worker not supported, but using canvas/bitmap. Rare case
            this.element.height = this.best.Height;
            this.element.width = this.best.Width;
            this.draw(this.preloader as HTMLImageElement, true);
          }
        }
        if (this.blob?.indexOf("blob:") === 0) URL.revokeObjectURL(this.blob);
        return;
      } else {
        if (
          this.blob?.indexOf("blob:") === 0 ||
          this.blob === "empty" ||
          this.drawer
        )
          // C
          ...
          if (!this.preloader) {
            if (params.length > 0 && params[0] instanceof ImageBitmap)
              try {
                ...
                console.warn("Failed closing bitmap:", error);
              }
            return;
          }
          if (this.drawer)
